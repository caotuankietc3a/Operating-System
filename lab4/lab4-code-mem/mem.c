  ...
  if (current_region != NULL) {
    ...
    if (free_regions == NULL) {
      ...
      if (current_region->pointer < free_regions->pointer) {
        ...
        if (current_region->pointer + current_region->size ==
            free_regions->pointer) {
        ...
        // new region will be put on somewhere in the middle or at the end of
        // the list
...
void *best_fit_allocator(unsigned int size) {
  ...
  struct mem_region *temp_region = NULL;
  do {
    ...
    if (temp_region == NULL) {
      ...
      if(current_region->size < temp_region->size){
        temp_region = current_region;
      }
    }
    current_region = current_region->next;
